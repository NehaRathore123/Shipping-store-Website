...
export var login=async (req,res,next)=>{
    var bidDetails=req.body;
    bidDetails={...bidDetails,"status":1};
    var bidList = await BidSchemaModel.find(bidDetails);
    var l=bidList.length;
    if(l!=0)
    {
      let payload={"subject":bidList[0].email};
      let key=rs.generate();
      let token=jwt.sign(payload,key);
      return res.status(201).json({"token":token,"bidDetails":userList[0]});
    }
    else
